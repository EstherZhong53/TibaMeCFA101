...
public class FriendWS {
	private static Map<String, Session> sessionsMap = new ConcurrentHashMap<>(); // JDK5，將同步鎖定的範圍縮小了(可以很多執行緒並行，只要key vlue索取的空間不同)
	Gson gson = new Gson();
	...
	public void onOpen(@PathParam("userName") String userName, Session userSession) throws IOException{
		...
		String text = String.format("Session ID = %s, connected; userName = %s%nusers: %s", userSession.getId(), userName, userNames); // %n為換行
