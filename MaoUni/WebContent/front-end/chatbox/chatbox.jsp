	  ...
	  webSocket.onmessage = function(e){
		...
		function render(sender, showMsg, timestamp) {
		      ...
		      let context = { 
		    	  	...
		    	  	sender === self ? chat.$chatHistoryList.append(template(context)) : chat.$chatHistoryList.append(templateResponse(context));
		    	  	chat.scrollToBottom();
			        chat.$textarea.val('');
		...
		function prompNews(sender) {
			let target = $(`li[data-name = \${sender}]`);
	
			if($(`li[data-name = \${sender}] .news`).text() === ""){
				let text = "<span class = 'news'>news</span>";
				target.append(text);
			}


// 			$(`li[data-name = \${sender}] .news`).text()
// 				let text = "<span class = 'news'>news</span>";
// 				target.append(text);
// 			}
		 ...
		 function refreshFriendList(jsonObj){  // 渲染好友列表
...
//		        			statusIcon: jsonObj.status // from redis
    ...
    getReceiver: function(e){				// 選取接收對象後取得歷史訊息
   		if(!e.target.tagName === "friend"){ return };
    	chat.receiver = e.target.dataset.name;
		chat.$chatHistoryList.html("");
		$(`li[data-name = \${chat.receiver}] .news`).text("");
    ...
    getCurrentTime: function() {
    	let now = new Date()
    	let t = now.toLocaleTimeString().replace(/([\d]+:[\d]{2})(:[\d]{2})(.*)/, "$1$3");
